...
class GurobiSolver(Solver):
    def __init__(self, model: Model | None = None) -> None:
        ...
        self.problem: GRBModel = GRBModel()  # pyright: ignore
    ...
    def add_variables(self, var_dict: dict) -> None:
        ...
        self.problem.update()  # pyright: ignore
    ...
    def add_constraints(self, constr_dict: dict) -> None:
        ...
        for constr_id, (lhs, sense, rhs) in constr_dict.items():
            ...
            self.problem.addLConstr(expr, sense_mapping[sense], rhs, constr_id)  # pyright: ignore
    ...
    def remove_constraint(self, constr_id: str) -> None:
        if constr_id in self.constraints:
            self.problem.remove(self.problem.getConstrByName(constr_id))  # pyright: ignore
        else:
    ...
    def set_objective(self, objective: str | dict, minimize: bool = True) -> None:
        ...
        self.problem.setObjective(obj_expr, sense)  # pyright: ignore

        self.objective = objective
        self.minimize = minimize

    def internal_solve(self) -> Status:
        self.problem.optimize()  # pyright: ignore
        status = status_mapping.get(self.problem.status, Status.UNKNOWN)  # pyright: ignore
        return status
    ...
    def get_solution(
        ...
        fobj = self.problem.ObjVal  # pyright: ignore
        ...
        if shadow_prices:
            ...
            for m_id in self.constraints:
                ...
                value = constr.Pi  # pyright: ignore
                s_prices[m_id] = value
        else:
            s_prices = None
    ...
    def set_temporary_bounds(self, bounds: dict) -> dict:
        ...
        for r_id, x in bounds.items():
            ...
            if r_id in self.variables:
                lpvar = self.problem.getVarByName(r_id)  # pyright: ignore
                old_constraints[r_id] = (lpvar.lb, lpvar.ub)  # pyright: ignore
                lpvar.lb = infinity_fix(lb)  # pyright: ignore
                lpvar.ub = infinity_fix(ub)  # pyright: ignore
            else:
    ...
    def reset_bounds(self, bounds: dict) -> None:
        ...
        for r_id, (lb, ub) in bounds.items():
            lpvar = self.problem.getVarByName(r_id)  # pyright: ignore
            lpvar.lb, lpvar.ub = lb, ub  # pyright: ignore
        self.problem.update()  # pyright: ignore
    ...
    def set_parameter(self, parameter: Parameter, value: float) -> None:
        ...
        if parameter in parameter_mapping:
            ...
            self.problem.setParam(grb_param, value)  # pyright: ignore
        else:
            raise Exception(f"Unknown parameter: {str(parameter)}")

    def set_logging(self, enabled: bool = False) -> None:
        self.problem.setParam("OutputFlag", 1 if enabled else 0)  # pyright: ignore

    def write_to_file(self, filename: str) -> None:
        self.problem.write(filename)  # pyright: ignore
